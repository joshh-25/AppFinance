...
function handle_property_actions($action)
{
    ...
    if ($action === 'property_record_delete' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            $linkedBillsStmt = $pdo->prepare(
                "SELECT COUNT(*)
                 FROM `property_billing_records`
                 WHERE `property_list_id` = ?
                   AND `is_hidden` = 0"
            );
            $linkedBillsStmt->execute([$id]);
            $linkedBillRows = (int) $linkedBillsStmt->fetchColumn();
            if ($linkedBillRows > 0) {
                if ($pdo->inTransaction()) {
                    $pdo->rollBack();
                }
                ...
                echo json_encode([
                    'success' => false,
                    'message' => 'This property cannot be deleted because monthly bill records are linked to it.',
                ]);
                return true;
            }
            ...
            audit_log_event('property_list_delete', [
                ...
                'linked_bill_rows' => $linkedBillRows,
            ]);
            echo json_encode([
                'success' => true,
                'message' => 'Property deleted from Property List.',
            ]);
        } catch (Throwable $e) {
            if (isset($pdo) && $pdo instanceof PDO && $pdo->inTransaction()) {
                $pdo->rollBack();
