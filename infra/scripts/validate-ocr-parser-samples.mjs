...
async function main() {
  ...
  const rows = samples.map((sample) => {
    ...
    const expectedBillType = cleanText(sampleObject.parser_expected_bill_type || sampleObject.expected_bill_type).toLowerCase();
    ...
    const expectedPresentFields = ensureArray(sampleObject.parser_expected_present_fields ?? sampleObject.expected_present_fields).map((field) => cleanText(field));
    const expectedValues = ensureObject(sampleObject.parser_expected_values ?? sampleObject.expected_values);
