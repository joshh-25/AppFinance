...
export default function ExpensesPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setFormFeedback({
        ...
        message: 'You are editing an existing expense record. Press Save Changes when you finish.'
      });
    } catch {
      window.sessionStorage.removeItem(EXPENSE_EDIT_CONTEXT_KEY);
    }
  ...
  function resetForm() {
    ...
    setFormFeedback({
      tone: 'info',
      title: 'Form Ready',
      message: 'The expense form is ready for a new entry.'
    });
    window.sessionStorage.removeItem(EXPENSE_EDIT_CONTEXT_KEY);
  }
  ...
  async function handleSave(event) {
    ...
    if (!hasRequiredFields(form)) {
      setFormFeedback({
        ...
        message: 'Date, payee, description, and amount are required.'
      });
    ...
    setFormFeedback({
      tone: 'info',
      title: isEditMode ? 'Saving Changes' : 'Saving Expense',
      message: 'Please wait while the expense record is being saved.'
    });
    setSaving(true);
    try {
      ...
      if (isEditMode) {
        ...
        setFormFeedback({
          tone: 'success',
          title: 'Changes Saved',
          message: 'Your expense changes were saved successfully.'
        });
        ...
        setFormFeedback({
          tone: 'success',
          title: 'Expense Saved',
          message: 'Your expense record was saved successfully.'
        });
      ...
      setFormFeedback({
        ...
        message: String(error?.message || 'Failed to save expense.')
      });
  ...
  function handleOcrButtonClick() {
    setFormFeedback({
      ...
      message: 'Choose one receipt or expense file to auto-fill the form.'
    });
  ...
  async function handleOcrFileSelect(event) {
    ...
    setFormFeedback({
      tone: 'info',
      title: 'Reading Expense File',
      message: `Please wait while ${file.name} is scanned.`
    });
    setOcrBusy(true);
    try {
      ...
      if (autoFilledCount > 0) {
        setFormFeedback({
          ...
          message: `Auto-filled ${autoFilledCount} field(s) from ${file.name}.`
        });
        ...
        setFormFeedback({
          ...
          message: 'No fields were confidently auto-filled. You can enter the values manually.'
        });
      ...
      setFormFeedback({
        tone: 'error',
        title: 'Upload Failed',
        message: String(error?.message || 'Upload processing failed.')
      });
      ...
      <section className="card bill-form-card expense-form-card">
          ...
          <div className="card-title-left">
            ...
            <div>
              <h3 className="card-title">
                Expense Entry <span className="mode-badge">{isEditMode ? 'Editing Expense' : 'New Expense'}</span>
              </h3>
            </div>
          </div>
          <div className="card-title-actions">
            <button
              ...
              onClick={() => {
                setFormFeedback({
                  tone: 'info',
                  title: 'Opening Expense List',
                  message: 'You are moving to the saved expense records list.'
                });
                navigate('/records/expenses');
              }}
            >
              Open List
            </button>
            ...
            >
              {ocrBusy ? 'Uploading Receipt...' : 'Upload Receipt'}
            </button>
            <input
              ref={ocrInputRef}
              type="file"
        ...
        <div className="form-section-card">
          <div className="form-section-header">
            ...
            <p>Fill in the expense information below, then save the record.</p>
          </div>
        ...
        <div className="workflow-footer">
          <div className="workflow-footer-right">
            ...
            >
              {isEditMode ? (saving ? 'Saving Changes...' : 'Save Changes') : saving ? 'Saving...' : 'Save Expense'}
            </button>
          </div>
        </div>
      </section>
