export const UPLOAD_REQUIRED_FIELDS_BY_TYPE = {
    internet: ['wifi_amount', 'internet_account_no', 'wifi_due_date'],
    water: ['water_amount', 'water_account_no', 'water_due_date'],
    electricity: ['electricity_amount', 'electricity_account_no', 'electricity_due_date'],
    association_dues: ['association_dues', 'association_due_date']
};

export const UPLOAD_WARNING_FIELDS_BY_TYPE = {
    internet: ['internet_provider', 'wifi_payment_status'],
    water: ['water_payment_status'],
    electricity: ['electricity_payment_status'],
...
function toFieldLabel(field) {
    return UPLOAD_FIELD_LABELS[field] || field;
}

function hasPropertySelection(data) {
    return (
        Number(data?.property_list_id || 0) > 0 ||
        cleanTextValue(data?.dd) !== '' ||
        cleanTextValue(data?.property) !== ''
    );
}
...
function normalizeDdValue(value) {
    ...
    return raw.replace(/\b(\d+\s+[A-Z]{2,}\s+\d)\s+([A-Z]{1,3})\b/g, '$1$2');
}
...
function parseUploadFieldsFromText(text) {
    ...
    const waterMeterLinePatterns = [
        /\bPrev[:\s]*[0-9]{1,3}(?:[, ]?[0-9]{3})*(?:\.\d{2})?\s*,?\s*Curr[:\s]*[0-9]{1,3}(?:[, ]?[0-9]{3})*(?:\.\d{2})?\s*,?\s*Used[:\s]*[0-9]{1,3}(?:[, ]?[0-9]{3})*(?:\.\d{2})?\s*,?\s*Rate[:\s]*[0-9]{1,3}(?:\.\d+)?\s*\/\s*cu\.?\s*m\.?\s*([0-9]{1,3}(?:[, ]?[0-9]{3})*(?:\.\d{2})|[0-9]+\.\d{2})/i
    ];
    const waterLineItemPatterns = [
        /\bWater\s*Cons(?:umption)?[^\n]*?([0-9]{1,3}(?:[, ]?[0-9]{3})*(?:\.\d{2})|[0-9]+\.\d{2})/i,
        /\bUtilities?\s*(?:Charge|Charges)?\s*-\s*Water(?!\s*Prev\s*:)[^\n]*?([0-9]{1,3}(?:[, ]?[0-9]{3})*(?:\.\d{2})|[0-9]+\.\d{2})/i
    ];
    const hasWaterLineItem = [...waterLineItemPatterns, ...waterMeterLinePatterns].some((pattern) => pattern.test(parsingText));

    return {
        bill_type:
            pickTextMatch(parsingText, [/\bBill\s*Type[:\s-]*([A-Za-z_ ]+)/i, /\bUtility\s*Type[:\s-]*([A-Za-z_ ]+)/i]) ||
            inferredBillType,
        billing_period: pickTextMatch(parsingText, [
            /\bBilling\s*(?:Period|Month)[:\s-]*([^\n]+?)(?:\s+(?:Property\/DD|Property|Due\s*Date|Account|Reference|Payment\s*Status)\b|$)/i,
            /\bDue\s*Period[:\s-]*([^\n]+?)(?:\s+(?:Property\/DD|Property|Due\s*Date|Account|Reference|Payment\s*Status)\b|$)/i
        ]),
        dd:
        ...
        water_amount: pickTextMatch(parsingText, [
            ...waterMeterLinePatterns,
            ...waterLineItemPatterns,
            /\bWater\s*(?:Amount|Charge|Charges|Dues?)?[^\n]*?([0-9]{1,3}(?:[, ]?[0-9]{3})*(?:\.\d{2})|[0-9]+\.\d{2})/i
        ]),
        water_line_detected: hasWaterLineItem,
...
export function validateUploadExtraction(data, requiredBillType) {
    ...
    const warningFields = UPLOAD_WARNING_FIELDS_BY_TYPE[billType] || [];
    const missingRequiredFields = requiredFields.filter((field) => cleanTextValue(data[field]) === '');
    const blockingSharedFields = [];
    if (!hasPropertySelection(data)) {
        blockingSharedFields.push('property');
    }
    if (cleanTextValue(data?.due_period) === '') {
        blockingSharedFields.push('due_period');
    }
    const missingWarningFields = warningFields.filter((field) => cleanTextValue(data[field]) === '');
    const confidence = deriveUploadConfidence(data, billType);

    if (missingRequiredFields.length === 0 && blockingSharedFields.length === 0) {
        const warnings = missingWarningFields.map((field) => UPLOAD_FIELD_LABELS[field] || field);
        const confidenceMessage =
            confidence.summary === 'medium' && confidence.heuristicFields.length > 0
                ? `OCR confidence: medium. Heuristic fields: ${confidence.heuristicFields.join(', ')}.`
                : '';
        const messageParts = [];
        if (warnings.length > 0) {
            messageParts.push(`OCR completed with warnings. Missing optional fields: ${warnings.join(', ')}.`);
        }
        if (confidenceMessage) {
            messageParts.push(confidenceMessage);
        }
        return {
            valid: true,
    ...
    const missingLabels = [...blockingSharedFields, ...missingRequiredFields].map(
        (field) => toFieldLabel(field)
    );
    ...
    const confidenceTail =
        confidence.summary === 'medium' && confidence.heuristicFields.length > 0
            ? ` Heuristic fields: ${confidence.heuristicFields.join(', ')}.`
            : '';
    return {
        valid: false,
        message: `OCR needs review before save. Missing fields: ${missingLabels.join(', ')}.${moduleMismatch}${confidenceTail}`,
        warnings: missingWarningFields.map((field) => UPLOAD_FIELD_LABELS[field] || field),
        missingFields: [...blockingSharedFields, ...missingRequiredFields],
        confidence
    };
}

export function deriveUploadConfidence(data, requiredBillType) {
    if (!data || typeof data !== 'object') {
        return {
            summary: 'low',
            exactFields: [],
    ...
    if (Number(data?.property_list_id || 0) > 0) {
        exactFields.push(toFieldLabel('property'));
        exactFields.push(toFieldLabel('dd'));
    } else if (cleanTextValue(data?.property) !== '' || cleanTextValue(data?.dd) !== '') {
        heuristicFields.push(toFieldLabel('property'));
        heuristicFields.push(toFieldLabel('dd'));
    } else {
        missingCriticalFields.push(toFieldLabel('property'));
    }

    if (cleanTextValue(data?.due_period) !== '') {
        exactFields.push(toFieldLabel('due_period'));
    } else {
        missingCriticalFields.push(toFieldLabel('due_period'));
    }

    requiredFields.forEach((field) => {
        if (cleanTextValue(data?.[field]) !== '') {
            exactFields.push(toFieldLabel(field));
        } else {
            missingCriticalFields.push(toFieldLabel(field));
        }
    });

    const uniqueExactFields = Array.from(new Set(exactFields));
    const uniqueHeuristicFields = Array.from(new Set(heuristicFields.filter((field) => !uniqueExactFields.includes(field))));
    const uniqueMissingCriticalFields = Array.from(new Set(missingCriticalFields.filter((field) => !uniqueExactFields.includes(field))));

    let summary = 'high';
    if (uniqueMissingCriticalFields.length > 0) {
        summary = 'low';
    } else if (uniqueHeuristicFields.length > 0) {
        summary = 'medium';
    }

    return {
        summary,
        exactFields: uniqueExactFields,
        heuristicFields: uniqueHeuristicFields,
        missingCriticalFields: uniqueMissingCriticalFields
    };
}
...
export function normalizeUploadData(input) {
    ...
    const normalizedBillTypeHint = (() => {
        const lowered = cleanTextValue(resolvedBillType || parsedFromText.bill_type || '').toLowerCase();
        ...
        if (lowered.includes('internet') || lowered.includes('wifi')) {
            return 'internet';
        }
    ...
    const explicitWaterAccountNo = pickUploadValue(
        lookup,
        ['water_account_no', 'waterAccountNo', 'wateracctno', 'wateracctnumber'],
        parsedFromText.water_account_no || ''
    );
    const parsedWaterLineAmount = normalizeAmountValue(parsedFromText.water_amount || '');
    const payloadWaterAmount = normalizeAmountValue(pickUploadValue(lookup, ['water_amount', 'waterAmount'], ''));
    const explicitWaterAmount = parsedWaterLineAmount !== '' ? parsedWaterLineAmount : payloadWaterAmount;
    const hasParsedWaterLineAmount =
        parsedFromText.water_line_detected === true ||
        cleanTextValue(parsedFromText.water_line_detected).toLowerCase() === 'true' ||
        parsedWaterLineAmount !== '';
    const explicitWaterDueDate = normalizeDateValue(
        pickUploadValue(lookup, ['water_due_date', 'waterDueDate'], parsedFromText.water_due_date || '')
    );
    const explicitWaterPaymentStatus = normalizePaymentStatusValue(
        pickUploadValue(lookup, ['water_payment_status', 'waterPaymentStatus'], parsedFromText.water_payment_status || '')
    );
    const hasExplicitWaterContext =
        cleanTextValue(explicitWaterAccountNo) !== '' ||
        explicitWaterDueDate !== '' ||
        explicitWaterPaymentStatus !== '' ||
        hasParsedWaterLineAmount;
    const trustExplicitWaterAmount =
        explicitWaterAmount !== '' &&
        (
            normalizedBillTypeHint === 'water' ||
            hasExplicitWaterContext
        );
    const allowGenericWaterFallback =
        normalizedBillTypeHint === 'water' ||
        cleanTextValue(explicitWaterAccountNo) !== '' ||
        trustExplicitWaterAmount;
    ...
    const waterDueDateExplicit = normalizeDateValue(
        pickUploadValue(lookup, ['water_due_date', 'waterDueDate'], parsedFromText.water_due_date || '')
    );
    const waterDueDateFallback = allowGenericWaterFallback
        ? normalizeDateValue(
              pickUploadValue(lookup, ['due_date', 'dueDate', 'current_bill_due_date', 'currentBillDueDate'], '')
          )
        : '';
    const waterDueDate = waterDueDateExplicit || waterDueDateFallback;
    const waterPaymentStatus =
        normalizePaymentStatusValue(
            pickUploadValue(lookup, ['water_payment_status', 'waterPaymentStatus'], parsedFromText.water_payment_status || '')
        ) ||
        (allowGenericWaterFallback ? normalizePaymentStatusValue(pickUploadValue(lookup, ['payment_status'], '')) : '');
    ...
    let result = {
        ...
        dd: normalizeDdValue(pickUploadValue(lookup, ['dd', 'property_dd', 'propertydd'], parsedFromText.dd || '')),
        property: pickUploadValue(lookup, ['property', 'property_name', 'propertyName'], parsedFromText.property || ''),
        billing_period: duePeriod,
        ...
        water_account_no: explicitWaterAccountNo,
        water_amount: waterAmount,
        water_due_date: waterDueDate,
    ...
    // Signal for Bill Review mixed-row generation: true only when OCR text contains
    // an explicit water line-item match (not generic total/due fallbacks).
    result.water_line_detected = hasParsedWaterLineAmount;
    ...
    result.ocr_confidence = deriveUploadConfidence(result, result.bill_type || detectBillTypeFromData(result));
