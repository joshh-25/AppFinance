...
import { getSessionQueryOptions, getStoredSessionRole, getStoredSessionUsername, logout } from '../lib/auth.js';
...
export default function AppLayout({
  ...
  const canOpenBillReview = canRoleAccessAction(currentRole, 'upload_bill');
  const canOpenExpensesEntry = canRoleAccessAction(currentRole, 'expense_create');

  useEffect(() => {
    const liveUsername = typeof sessionData?.username === 'string' ? sessionData.username.trim() : '';
    const stored = getStoredSessionUsername().trim();
    const nextName = liveUsername || stored;
    setDisplayName(nextName === '' ? 'User' : nextName);
  }, [location.pathname, location.search, sessionData?.username]);
  ...
  async function handleLogout(event) {
    ...
    try {
      ...
      window.location.assign(getRuntimeConfig().loginPath || '/login');
    }
  }
  ...
  return (
    <div className="shell">
      ...
      <aside className={`shell-sidebar ${isSidebarOpen ? 'open' : ''}`}>
        ...
        <button type="button" className="shell-user shell-user-button" onClick={handleLogout}>
