// Finance App File: frontend\src\pages\__tests__\BillingFlow.integration.test.jsx
...
import { clearGlobalEditMode } from '../../lib/globalEditMode.js';
import { createBill, fetchBills, fetchMergedBills, fetchPropertyRecords, updateBill, uploadBill } from '../../lib/api.js';
...
vi.mock('../../lib/api.js', () => ({
  fetchBills: vi.fn(async (options = {}) => {
    ...
    const billType = String(options.billType || '').trim().toLowerCase();
    ...
    let rows = [...mockBills];
    if (billType) {
      rows = rows.filter((row) => String(row.bill_type || '').trim().toLowerCase() === billType);
    }
    if (search) {
